...
import es.us.investigacion.parkinson.data.EventInProgress_Table;

/**
 * Created by LuisMiguel on 24/06/2016.
 */
public class SessionInProgressAdapter extends RecyclerView.Adapter<SessionInProgressAdapter.EventRowViewHolder> {
    private Context context;
    private FlowCursorList<EventInProgress> cursor;
    private int lastPosition = -1;

    public SessionInProgressAdapter(Context context) {
        this.context = context;
        FlowContentObserver observer = new FlowContentObserver();
        observer.registerForContentChanges(context, EventInProgress.class);
    ...
    public void refreshData() {
        new AsyncTask<Void, Void, FlowCursorList<EventInProgress>>() {
            @Override
            protected FlowCursorList<EventInProgress> doInBackground(Void... voids) {
                return new Select().from(EventInProgress.class).orderBy(EventInProgress_Table.timeAbsolute, false).cursorList();
            }

            @Override
            protected void onPostExecute(FlowCursorList<EventInProgress> result) {
                SessionInProgressAdapter.this.cursor = result;
                SessionInProgressAdapter.this.notifyDataSetChanged();
            }
        }.execute();
    ...
    public void onBindViewHolder(final EventRowViewHolder rowViewHolder, final int position) {
        final EventInProgress item = cursor.getItem(position);
